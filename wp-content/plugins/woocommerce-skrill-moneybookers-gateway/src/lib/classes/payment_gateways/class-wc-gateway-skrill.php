...
class WC_Gateway_Skrill extends WC_Payment_Gateway {
	...
	/**
	 * Indicates if the test gateway should be used.
	 *
	 * @return bool
	 * @since 1.3.1.151017
	 */
	protected function use_test_gateway() {
	...
	public function __construct() {
		...
		//$this->default_to_cc_payment = $this->get_option('default_to_cc_payment');
		// TODO Review "default to CC payment" feature. It's used to disable the login form, but there must be a better way to do it
		$this->default_to_cc_payment = 'no';
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled' => array(
				...
				'type' => 'checkbox',
				'label' => __('Enable Skrill (Moneybookers) checkout', $this->text_domain),
				'default' => 'yes'
			),
			'title' => array(
				...
				'default' => __('Credit Card (Skrill)', $this->text_domain),
				'desc_tip' => true,
			),
			'description' => array(
				...
				'default' => __('Pay securely with your credit card.', $this->text_domain)
			),
			'email' => array(
				'title' => __('Skrill Email', $this->text_domain),
				'type' => 'text',
				'description' => __('The email address associated with your Skrill account.', $this->text_domain),
				'default' => '',
				'desc_tip' => true,
			),
			'secret_word' => array(
				'title' => __('Secret Word', $this->text_domain),
				'type' => 'text',
				'description' => __('The Secret Word that will be used to authenticate the communication with Skrill.', $this->text_domain),
				'default' => '',
				'desc_tip' => true,
			),
			'default_language' => array(
				'title' => __('Default language for Skrill portal', $this->text_domain),
				'type' => 'select',
				'description' => __('This will be the default language which will be used to display the Skrill portal to ' .
														'your customers, if they choose a language that Skrill does not support.', $this->text_domain),
				'default' => '',
				'options' => $this->supported_languages,
			),
			//'default_to_cc_payment' => array(
			//	'title' => __('Default to credit card payment instead of Skrill login', $this->text_domain),
			//	'type' => 'checkbox',
			//	'description' => __('When this option is enabled, the email address entered by the ' .
			//											'customer at checkout will not be passed to the Skrill portal. ' .
			//											'This will result in Skrill presenting the credit card payment ' .
			//											'form, instead of the login form.', $this->text_domain),
			//	'default' => 'no',
			//	'options' => $this->supported_languages,
			//),
			'testing' => array(
				'title' => __('Gateway Testing', $this->text_domain),
				'type' => 'title',
				'description' => '',
			),
			'test_mode' => array(
				'title' => __('Use Skrill test portal', $this->text_domain),
				'type' => 'checkbox',
				'label' => __('Enable Skrill test portal', $this->text_domain),
				'default' => 'yes',
				'description' => __('Skrill test portal can be used to test payments.', $this->text_domain),
			),
			'debug' => array(
				'title' => __('Debug Log', $this->text_domain),
				'type' => 'checkbox',
				'label' => __('Enable debug mode', $this->text_domain),
				'default' => 'yes',
				'description' => sprintf(__('Enable debug mode and log Skrill events to file ' .
																		'<code>%s</code>.', $this->text_domain),
																 $log_file_path),
			),
		);
	}
	...
	protected function get_billing_fields(WC_Order $order) {
		...
		// If the "default to credit card payment" option is disabled, send customer's
		// email address as well. This makes Skrill show the "login to Skrill" by
		// default. When the same option is enabled, the email address is not sent,
		// and Skrill presents the "pay with credit card" form instead
		if($this->default_to_cc_payment != 'yes') {
			$billing_fields['pay_from_email'] = $order->billing_email;
		}
	...
	protected function get_skrill_args(WC_Order $order) {
		$this->log(sprintf(__('Skrill - Generating payment form for order #%s (order ID: %s).'),
		...
		$skrill_args = array(
			...
			// Order details
			'merchant_fields' => 'order_key,order_id',
			'order_key' => $order->order_key,
			'order_id' => $order->id,
			'transaction_id' => $order->get_order_number(),
			'amount' => $order_total,
			'amount2' => $order_total,
			'amount2_description' => sprintf(__('Order #%s', $this->text_domain), $order->get_order_number()),
	...
	protected function is_payment_successful($posted_data) {
		...
		$order_id = get_value('order_id', $posted_data);
		$order = new Aelia_Order($order_id);
